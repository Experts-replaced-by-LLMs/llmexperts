...
from src.llmexperts.scale import scale_file, ensure_output_paths
...
def test_scale_file(output_folder, summary_file_folder):
    """
    examples = [True, False]
    personas = [None, 0, [0,1]]
    ...
    skip_existing_scale_res = [True, False]
    """
    ...
    args = dict(
        ...
        save_log = True,
        meta_columns = None, skip_existing_scale_results = True,
        dry_run=True
    )
    ...
    for filename in filenames:
        ...
        scale_file(file_path, **args)

    df = pd.read_csv(os.path.join(output_folder, "scale_results.csv"))
...
def test_scale_file_use_examples(output_folder, summary_file_folder):
    model_list = ["gpt-4o-2024-08-06", "claude-3-5-sonnet-20241022", "gemini-1.5-pro-002"]
    issue_list = ["issue_1", "issue_2"]
    prompt_template = os.path.join(os.path.dirname(__file__), "prompts-scale.yaml")
    args = dict(
